...
export class TextArea extends Widget {
  ...
  constructor(args: IWidget) {
    ...
    args.style = "bg-gray-900 text-gray-400 rounded-md px-4 py-2 focus:outline-none focus:ring focus:ring-blue-500 focus:border-blue-500"
    if (args.style) {
      args.style.split(" ").map(el => this.element.classList.add(el))
    }
    
    if (Graph.widgetRootElementId != "")
      document.getElementById(Graph.widgetRootElementId).appendChild(this.element);
    else
  ...
  render(): void {
    ...
    AdjustElementPos(this.element, this.pos, this.width, this.height, this.fontSize);
  }
